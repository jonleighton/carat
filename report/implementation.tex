...
\code{Carat::Runtime} is the fundamental class responsible for executing a program. When initialized it sets up the runtime environment by:

\begin{enumerate}
  \item Initialising the stacks. Three stacks are used:
    \begin{enumerate}
    ...
    \end{enumerate}
  \item Creating a hash mapping the names of constants to their values. Constants are globally defined, so are not stored by scopes
  \item Running the \code{KernelLoader} which sets up the core classes in the target language (\code{Object}, \code{Class}, \code{Array}, \code{String}, ...). While some code is needed in the implementation language, most of this is done by interpreting class definitions written in the target language. To optimise this, the parsing is done ahead-of-time and the AST nodes are stored as binary data in files. These files are then loaded directly.
\end{enumerate}
