...
The parser produces a parse tree which consists of \code{Treetop::Runtime::SyntaxNode} objects. The class to be used is specified by `\code{<WhileExpression>}' at the end. Each different type of node has a \code{to_ast} method, which is used to recursively convert the parse tree to an AST made up of \code{Carat::AST::Node} objects.
