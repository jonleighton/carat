...
Scopes can have a \defn{block} attribute. If a block is given in the argument list of a method call, it is assigned as the block of the scope within which the call is evaluated.

\code{Kernel#yield} can then be used to call the block. For example:
...
All exception are instances of the \code{Exception} class or a subclass. Exceptions have a \defn{message} which describes the problem. They are raised by a call to \code{Kernel#raise}, which can take various forms:
...
\begin{enumerate}
  \item Raises a \code{SomeException} exception with the message ``there's a problem"
  \item Ditto
  \item Raises a \code{SomeException} exception with the message ``(no message)"
  \item Raises a \code{RuntimeError} exception with the message ``there's a problem"
  \item Raises a \code{RuntimeError} exception with the message ``(no message)"
\end{enumerate}

The \code{Kernel#raise} method standardises the various different argument forms and then passes the exception object to the \code{raise} primitive, which:
...
The \defn{default failure continuation} is at the very bottom of the stack. It prints out the exception class name, message, and the backtrace. For example, the program:
...
When a \code{Begin} node is evaluated, it puts a new failure continuation in a frame on the stack. Then, when the failure continuation is called, it:
