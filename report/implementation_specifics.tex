...
Lambdas are created by the \code{Kernel#lambda} method (or \code{Lambda.new}):

\begin{lstlisting}
lambda { |x| x + x }.call(2) # => 4
\end{lstlisting}

The literal block is parsed as an item in the argument list. When evaluated, the expression inside the block item is a \code{Block} AST node. When a \code{Block} node is a evalauted, it creates a new \code{Lambda} object.

So it is not quite create to say that the \code{lambda} method \textit{creates} a lambda; that is done implicitly when a block is given in an argument list.
...
The \code{lambda} method, then, just returns the block from the current scope, which will be the one created when making the call.
